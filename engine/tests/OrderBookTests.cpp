...
{
protected:
    std::shared_ptr<BlockingQueue<aether_market_data::OrderDelta>> blockingQueue;
    std::shared_ptr<OrderBook> book;

    void SetUp() override {
        blockingQueue = std::make_shared<BlockingQueue<aether_market_data::OrderDelta>>();
        book = std::make_shared<OrderBook>(blockingQueue);
    }
};
